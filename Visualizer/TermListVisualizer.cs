	...
	public class TermListVisualizer
	{
			...
			GOTerm[] terms, BDictionary<int, GOTermProperties> properties, ProgressEventHandler progressHandler)
		{
			...
			if (this.aTerms.Length > 0)
			{
				ConstructMatrix();
				MakeOntologram();
				MakeSimpleThresholdOntolograph(0.97);
			}
		}
		...
		private void ConstructMatrix()
		{
			...
			this.oMatrix = new TermSimilarityMatrix(this, myMatrix_OnProgress);
		}
		...
		private void MakeOntologram()
		{
			...
			this.oMatrix.CalculateUniqueness();
			...
			this.BuildDispensableTermList(oRnd);
		...
		private void BuildDispensableTermList(RandomMT19937 rnd)
		{
			GeneOntology oOntology = this.oParent.Ontology;
			SpeciesAnnotations oAnnotations = this.oParent.Annotations;
			 ...
			 */
			/*for (int i = 0; i < oSimilarityMatrix.Count; i++)
			{
				int iXID = oSimilarityMatrix[i].Key;
				BDictionary<int, double> oYAxis = oSimilarityMatrix[i].Value;
				double propX = oTermProperties.GetValueByKey(iXID).TransformedValue;
				if (double.IsNaN(propX))
					continue;

				// we are symmetrical
				for (int j = i + 1; j < oYAxis.Count; j++)
				{
					int iYID = oYAxis[j].Key;

					if (iXID == iYID)
						continue;

					double propY = oTermProperties.GetValueByKey(iYID).TransformedValue;
					if (double.IsNaN(propY))
						continue;

					// GO categories with enrichment of different signs cannot eliminate one another
					if (Math.Sign(propX) != Math.Sign(propY))
					{
						oYAxis.SetValueByKey(iYID, -1.0);
						//oSimilarityMatrix.GetValueByKey(iYID).SetValueByKey(iXID, -1.0); // symmetry!
					}
				}
			}*/
			...
			/*double dProgressStep = this.dProgressSlice / (double)oSimilarityMatrix.Terms.Count;
			int iRemovedCount = 0;

			while (oSimilarityMatrix.Count > 0)
			{
				// determine the maximum remaining pairwise similarity in the matrix
				double dMaxRemainingSimilarity = -1.0;
				int iXSelectedID, iYSelectedID;
				List<int[]> aSelectedIDPairs = new List<int[]>();
				...
				for (int i = 0; i < oSimilarityMatrix.Count; i++)
				{
					...
					for (int j = i + 1; j < oYAxis.Count; j++)
					{
						...
						if (iXID != iYID)
						{
							double dSimilarity = Math.Round(oYAxis[j].Value, 8);

							if (dSimilarity > -1.0)
							{
								if (dSimilarity > dMaxRemainingSimilarity)
								{
									dMaxRemainingSimilarity = dSimilarity;
									aSelectedIDPairs.Clear();
									aSelectedIDPairs.Add(new int[] { iXID, iYID });
								}
								else if (dSimilarity == dMaxRemainingSimilarity)
								{
									// when symmetry is accounted for no need for this check
									//bool bFound = false;
									//for (int k = 0; k < aSelectedIDPairs.Count; k++)
									//{
										//if ((aSelectedIDPairs[k][0] == iXID && aSelectedIDPairs[k][1] == iYID) ||
											//(aSelectedIDPairs[k][0] == iYID && aSelectedIDPairs[k][1] == iXID))
										//{
											//bFound = true;
											//break;
										//}
				...
				if (dMaxRemainingSimilarity == -1.0) // == -1.0
					break; // this will end loop

				if (aSelectedIDPairs.Count > 1)
				{
					int iSelected = rnd.Next(aSelectedIDPairs.Count);
					//writer.WriteLine(string.Format("Selected: {0}", iSelected));
					iXSelectedID = aSelectedIDPairs[iSelected][0];
					iYSelectedID = aSelectedIDPairs[iSelected][1];
				}
				else
				{
					iXSelectedID = aSelectedIDPairs[0][0];
					iYSelectedID = aSelectedIDPairs[0][1];
				}
				...
				// first condition - if one of the GO categories is "pinned" by the
				// user, it automatically wins the contest
				bool bXPinned = oTermProperties.GetValueByKey(iXSelectedID).Pinned;
				bool bYPinned = oTermProperties.GetValueByKey(iYSelectedID).Pinned;
				if (bXPinned && !bYPinned)
				{
					iRemoveID = iYSelectedID;
					goto decideWhichToRemove;
				}
				if (bYPinned && !bXPinned)
				{
					iRemoveID = iXSelectedID;
					goto decideWhichToRemove;
				}

				// second condition - if one GO category is 'very general' and the other
				// GO category is not, the 'very general' category automatically loses
				double proportionX = oAnnotations.GetFreqGuessIfUnknown(iXSelectedID, oOntology);
				double proportionY = oAnnotations.GetFreqGuessIfUnknown(iYSelectedID, oOntology);
				...
				// now, check the properties to see which term we like best
				double propX = oTermProperties.GetValueByKey(iXSelectedID).TransformedValue;
				double propY = oTermProperties.GetValueByKey(iYSelectedID).TransformedValue;

				if (double.IsNaN(propX) || double.IsNaN(propY))
				{
					propX = oTermProperties.GetValueByKey(iXSelectedID).Uniqueness;
					propY = oTermProperties.GetValueByKey(iYSelectedID).Uniqueness;
				}
			...
			if (propX > propY)
			{
				iRemoveID = keepGreater ? iYSelectedID : iXSelectedID;
			}
			else if (propX < propY)
			{
				iRemoveID = keepGreater ? iXSelectedID : iYSelectedID;
			}
			else
			{
				// they are equal.. here the fun begins :)
				...
				// also get sizes of X and Y, we'll need this to decide which to keep
				double sizeOfX = oAnnotations.GetSizeGuessIfUnknown(iXSelectedID, oOntology);
				double sizeOfY = oAnnotations.GetSizeGuessIfUnknown(iYSelectedID, oOntology);

				// parentsOfX.Contains(oOntology[iYSelectedID])
				if (oOntology.GetValueByKey(iXSelectedID).IsChildOf(iYSelectedID))
				{
					/* Y is a parent of X. Now check if Y is constituted mostly of X
					 * by comparing their log-sizes - if yes, keep X (the more
					 * specific term). Otherwise, keep Y, the parent term.
					 ...
					 */
			/*if ((sizeOfY - sizeOfX) / sizeOfY < 0.25)
				iRemoveID = iYSelectedID; // Y constituted mostly of X
			else
				iRemoveID = iXSelectedID; // Y constituted only partly of X
		}
		else if (oOntology.GetValueByKey(iYSelectedID).IsChildOf(iXSelectedID)) // parentsOfY.Contains(oOntology[iXSelectedID])
		{
			// X is a parent of Y. Rule as above, only reversed.
			if ((sizeOfX - sizeOfY) / sizeOfX < 0.25)
				iRemoveID = iXSelectedID; // X constituted mostly of Y
			else
				iRemoveID = iYSelectedID; // X constituted only partly of Y
		}
		else
		{
			// X and Y are not parent-child related - keep the more
...
decideWhichToRemove:
	...
	int dispensedById = (iRemoveID == iXSelectedID ? iYSelectedID : iXSelectedID);

	oTermProperties.GetValueByKey(iRemoveID).Dispensability = dMaxRemainingSimilarity;
	oTermProperties.GetValueByKey(iRemoveID).DispensedBy = dispensedById;

	// remove removed GOID from the matrix in X and Y axis
	for (int i = 0; i < oSimilarityMatrix.Count; i++)
	{
		if (oSimilarityMatrix[i].Key == iRemoveID)
		{
			oSimilarityMatrix.RemoveAt(i);
			i--;
		}
		else
		{
			oSimilarityMatrix[i].Value.RemoveByKey(iRemoveID);
		}
	}

	iRemovedCount++;
	double dProgress = this.dProgressPos + dProgressStep * (double)iRemovedCount;
	if (this.OnProgress != null)
		this.OnProgress(this, new ProgressEventArgs(dProgress));
		...
		private void MakeSimpleThresholdOntolograph(double similarityThreshold)
		{
